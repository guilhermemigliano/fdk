...
export default async function AdminPartidasPage() {
  ...
  return (
    <div className="flex min-h-svh w-full justify-center">
      {/* Wrapper central */}
      <div className="flex flex-col w-full max-w-3xl p-6">
        ...
        <div className="flex-1 overflow-y-auto space-y-6 pb-12">
          {meses.map((mes) => (
            <div key={mes} className="flex flex-col items-center gap-3">
              {/* Cabeçalho do mês */}
              <h2 className="text-xl font-bold text-center">{mes}</h2>
              ...
              <div className="w-full space-y-4">
                {agrupado[mes].map((match) => (
                  <Link
                    ...
                    className={`flex justify-between items-center w-full border rounded-lg p-4 hover:bg-muted transition
                    ${
                      match.isClosed
